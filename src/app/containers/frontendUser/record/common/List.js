...
import {
  ...
  deleteRecord,
  updateRecord,
} from "../../../../../core/actions/Records.action";
...
const List = (props) => {
  ...
  const getRecords = async () => {
    props
      .getRecords(currentObject._id, activeApplication._id)
      .then((response) => {
            ...
            columns.map((column) => {
              if (column.index === 0) {
                columnsArray.push({
                  ...
                  text: column.field && column.field.name,
                  hidden: true,
                  ...
                  formatter: (cell, row) =>
                    row && (
                      <CustomButton
                        ...
                        onClick={() =>
                          props.history.push(
                            `/user/${activeApplication._id}/${currentObject._id}/record/view/${row._id}`
                          )
                        }
                      />
                    ),
              ...
              formatter: (cell, row) => (
                <DropdownButton id="dropdown-action" title="">
                  <Dropdown.Item
                    onClick={() =>
                      props.history.push(
                        `/user/${activeApplication._id}/${currentObject._id}/record/edit/${row._id}`
                      )
                    }
                  >
                    Edit
                  </Dropdown.Item>
                  <Dropdown.Item onClick={() => handleDeleteRecord(row)}>
                    Delete
                  </Dropdown.Item>
                </DropdownButton>
              ),
  ...
  const renderAlphabetFilterOptions = () => {
    return alphabets.map((char) => (
      <li
        className={char === activeAlphabet ? "active" : null}
  ...
  const handleFilter = (char) => {
    ...
    const recordsFilter = recordList.filter(
      (item) => item[columnsID].charAt().toLowerCase() === char.toLowerCase()
    );
  ...
  const beforeSaveCell = async (oldValue, newValue, row, column, done) => {
    ...
    await props.updateRecord(row._id, updatedCellDetails).then((response) => {
      if (response && response.status === 200) {
        const {
          data,
          data: { result },
  ...
  return (
    <Container fluid>
      <Row
        ...
        className="p-3 mb-3 recordHead"
      >
        <Col xs={4} className="">
          <Row>
            <Col xs={1}>
              <img
                src="http://jbwork.in/jbplatform-images/images/projectIcon.png"
                alt=""
                className="recordNameIcon"
              />
            </Col>
            <Col xs={11}>
              <span className="recordName">{currentObject.label}</span>
              <span className="recordSubName">
                ({records.length} Item(s) found)
              </span>
            </Col>
          </Row>
        </Col>
        ...
        <Col xs={8} style={{ display: "flex", justifyContent: "flex-end" }}>
          <CustomButton
            ...
            onClick={() =>
              props.history.push(
                `/user/${activeApplication._id}/${currentObject._id}/record/create`
              )
            }
          />
        </Col>
      ...
      <Row>
        <Col className="dataTableMainWrap">
          <ul className="alphabetWrap">{renderAlphabetFilterOptions()}</ul>
          {/**
